...
export function Board() {
  ...
  const move = useAppStore((s) => s.moveApplication);

  const grouped = columns.reduce<Record<ApplicationStatus, typeof apps>>((acc, c) => {
    acc[c.id] = apps.filter((a) => a.status === c.id);
    return acc;
  }, {} as any);

  const onDragEnd = (r: DropResult) => {
    if (!r.destination) return;
    const newStatus = r.destination.droppableId as ApplicationStatus;
    move(r.draggableId, newStatus);
  };
          ...
          <div key={c.id} className="flex flex-col min-w-[280px]">
            <div className="flex items-center justify-between mb-3 px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground tabular-nums">{grouped[c.id].length}</span>
              </div>
